...
const AppointmentsContainer = ({ user, BASE_URL, appointments, activeDay, setActiveDay }) => {
  ...
  return (
    <ol className="flex flex-col bg-[#f7f6e7] w-72 gap-2 overflow-auto text-center pt-3 animate-sideBarSlideIn">
      {sortedAppointmentsAsc.map(appointment => <Appointment appointment={appointment} activeDay={activeDay} setActiveDay={setActiveDay} key={appointment._id} />)}
    </ol>
  );  
}
