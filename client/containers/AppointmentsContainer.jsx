...
const AppointmentsContainer = ({ user, BASE_URL, appointments, setActiveAppointmentId }) => {
  ...
  return (
    <ol className="flex flex-col bg-[#f7f6e7] w-72 gap-2 overflow-auto text-center pt-3">
      {sortedAppointmentsDesc.map(appointment => <Appointment appointment={appointment} setActiveAppointmentId={setActiveAppointmentId} key={appointment._id} />)}
    </ol>
  );  
}
